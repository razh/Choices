...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1_symbol_observable__ = __webpack_require__(8);
...
class Choices {
      ...
      const values = [...args],
        ...
        if (itemType === 'Object') {
          ...
          if (passedElementType !== 'text') {
            this._addChoice(item.value, item.label, true, false, -1, item.customProperties);
          } else {
            this._addItem(item.value, item.label, item.id, undefined, item.customProperties);
          }
        } else if (itemType === 'String') {
          if (passedElementType !== 'text') {
            this._addChoice(item, item, true, false, -1);
          } else {
            this._addItem(item);
          }
        }
        ...
        if (choices && choices.length) {
          ...
          choices.forEach((result, index) => {
            if (result.choices) {
              ...
              this._addChoice(result[value], result[label], result.selected, result.disabled, undefined, result['customProperties']);
            }
          });
        }
      }
  ...
  _addGroup(group, id, valueKey = 'value', labelKey = 'label') {
    ...
    if (groupChoices) {
      ...
      groupChoices.forEach(option => {
        ...
        let label = (0, _utils.isType)('Object', option) ? option[labelKey] : option.innerHTML;

        this._addChoice(option[valueKey], label, option.selected, isOptDisabled, groupId, option.customProperties);
      });
    } else {
  ...
  _createTemplates() {
    ...
    const templates = {
      ...
      itemList: () => {
        const localClasses = (0, _classnames2.default)(globalClasses.list, {
      ...
      item: data => {
        let localClasses = (0, _classnames2.default)(globalClasses.item, {
        ...
        if (this.config.removeItemButton) {
          localClasses = (0, _classnames2.default)(globalClasses.item, {
            [globalClasses.highlightedState]: data.highlighted,
      ...
      choiceGroup: data => {
        let localClasses = (0, _classnames2.default)(globalClasses.group, {
      ...
      choice: data => {
        let localClasses = (0, _classnames2.default)(globalClasses.item, globalClasses.itemChoice, {
          [globalClasses.itemDisabled]: data.disabled,
          [globalClasses.itemSelectable]: !data.disabled
        });
      ...
      input: () => {
        let localClasses = (0, _classnames2.default)(globalClasses.input, globalClasses.inputCloned);
      ...
      dropdown: () => {
        let localClasses = (0, _classnames2.default)(globalClasses.list, globalClasses.listDropdown);
        ...
        allChoices.forEach((choice, index) => {
          ...
          if (this.passedElement.type === 'select-one') {
            if (hasSelectedChoice || !hasSelectedChoice && index > 0) {
              ...
              this._addChoice(choice.value, choice.label, choice.selected, choice.disabled, undefined, choice.customProperties);
            } else {
              // Otherwise pre-select the first choice in the array
              this._addChoice(choice.value, choice.label, true, false, undefined, choice.customProperties);
            }
          } else {
            this._addChoice(choice.value, choice.label, choice.selected, choice.disabled, undefined, choice.customProperties);
          }
...
var _items = __webpack_require__(13);

var _items2 = _interopRequireDefault(_items);

var _groups = __webpack_require__(14);
...
const addItem = exports.addItem = (value, label, id, choiceId, groupId, customProperties) => {
  return {
    type: 'ADD_ITEM',
    value,
    label,
...
const addChoice = exports.addChoice = (value, label, id, groupId, disabled, elementId, customProperties) => {
  return {
    type: 'ADD_CHOICE',
    value,
    label,
 ...
 */
const generateChars = exports.generateChars = function (length) {
  var chars = '';
 ...
 */
const generateId = exports.generateId = function (element, prefix) {
